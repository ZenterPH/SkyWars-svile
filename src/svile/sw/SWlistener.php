...
class SWlistener extends Pluginbase implements Listener
{
    ...
    public function onSignChange(SignChangeEvent $ev)
    {
        ...
        if (!array_key_exists($SWname, $this->pg->arenas)) {
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'This arena doesn\'t exist, try ' . TextFormat::WHITE . '/sw create');
            return;
        }

        //Checks if a sign already exists for the arena
        if (in_array($SWname, $this->pg->signs)) {
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'A sign for this arena already exist, try ' . TextFormat::WHITE . '/sw signdelete');
            return;
        }
        ...
        foreach ($this->pg->arenas as $name => $arena) {
            if ($world == $arena->getWorld()) {
                $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'You can\'t place the join sign inside arenas');
                return;
            }
        }
        ...
        if (!$this->pg->arenas[$SWname]->checkSpawns()) {
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'Not all the spawns are set in this arena, try ' . TextFormat::WHITE . ' /sw setspawn');
            return;
        }

        //Saves the sign
        if (!$this->pg->setSign($SWname, ($ev->getBlock()->getX() + 0), ($ev->getBlock()->getY() + 0), ($ev->getBlock()->getZ() + 0), $world))
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'An error occured, please contact the developer');
        else
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::GREEN . 'SW join sign created !');
    ...
    public function onBreak(BlockBreakEvent $ev)
    {
        ...
        if (array_key_exists($key, $this->pg->signs)) {
            ...
            $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::GREEN . 'Arena reloaded !');
            if ($this->pg->setSign($this->pg->signs[$key], 0, 0, 0, 'world', true, false)) {
                $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::GREEN . 'SW join sign deleted !');
            } else {
                $ev->getPlayer()->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'An error occured, please contact the developer');
            }
        }
